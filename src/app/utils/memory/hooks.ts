...
export function useMemory(options: UseMemoryOptions = {}) {
  ...
  // 메모리 최적화 실행
  const runMemoryOptimization = useCallback(
    async (level?: number) => {
      try {
        setLoading(true);
...
export function useMemorySettings() {
  ...
  const saveSettings = useCallback(
    (newSettings: Partial<MemorySettings>) => {
      try {
        setLoading(true);
 ...
 */
export function useAutoMemoryOptimization(
  options: {
    enabled?: boolean;
    threshold?: number;
    interval?: number;
  ...
  const { enabled = true, threshold = 80, interval = 60000, showNotifications = true } = options;
 ...
 * 포맷된 메모리 정보를 제공하는 훅
 */
export function useFormattedMemoryInfo(
  options: {
    autoFetch?: boolean;
    interval?: number;
  } = {}
) {
  const { autoFetch = true, interval = 30000 } = options;

  const { memoryInfo, loading, error, fetchMemoryInfo } = useMemory({ autoFetch, interval });
...
export function useMemoryStatus(
  ...
  const { memoryInfo, loading, error, fetchMemoryInfo } = useMemory({ autoFetch: true });
