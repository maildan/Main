...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log('수신된 메모리 설정 업데이트 요청 (update route):', settings); // 로그 추가
    ...
    // 임시로 항상 성공 반환
    const success = true;
    console.warn('메모리 설정 업데이트 API (update route)는 현재 구현되지 않았습니다.');

    return NextResponse.json({ success, timestamp: Date.now() });
  } catch (error) {
    console.error('메모리 설정 업데이트 API (update route) 오류:', error);
