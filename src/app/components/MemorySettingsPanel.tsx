...
import { MemorySettings } from '@/types/memory-settings'; // 올바른 경로로 수정
import { runComprehensiveBenchmark } from '../utils/performance-metrics';
// 메모리 유틸리티 가져오기
import { setupPeriodicMonitoring } from '../utils/memory/utils'; // 대체 함수 사용
...
const MemorySettingsPanel: React.FC<MemorySettingsPanelProps> = ({
  ...
  useEffect(() => {
    const initializeSettings = async () => {
      try {
        // 메모리 유틸리티 초기화
        setupPeriodicMonitoring(() => { }, 60000); // 대체 함수 사용
