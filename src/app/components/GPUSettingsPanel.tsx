...
export function GPUSettingsPanel({
  ...
  onSettingsChange,
  onRestartNeeded
}: GPUSettingsPanelProps): React.ReactNode {
  ...
  useEffect(() => {
    async function loadGPUInfo() {
      try {
        ...
        addToast('GPU 정보를 가져오는 중 오류가 발생했습니다', 'error');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      if (success) {
        addToast('GPU 설정이 저장되었습니다', 'success');
        ...
        addToast('GPU 설정 저장에 실패했습니다', 'error');
      }
    } catch (error) {
      console.error('설정 저장 오류:', error);
      addToast('설정을 저장하는 중 오류가 발생했습니다', 'error');
    }
  };
  ...
  const applyOptimalSettings = async () => {
    try {
      ...
      addToast('최적 GPU 설정이 적용되었습니다. 저장하려면 저장 버튼을 클릭하세요.', 'info');
    } catch (error) {
      console.error('최적 설정 적용 오류:', error);
      addToast('최적 설정을 적용하는 중 오류가 발생했습니다', 'error');
    }
  };
