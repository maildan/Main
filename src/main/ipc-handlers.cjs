...
const { appState, HIGH_MEMORY_THRESHOLD } = require('./constants.cjs');
const { detectBrowserName, isGoogleDocsWindow } = require('./browser.cjs');
const { startTracking, stopTracking, saveStats, resetStats } = require('./stats.cjs');
const { debugLog } = require('./utils.cjs');
const { applyWindowMode } = require('./settings.cjs');
const { saveSettings, getSettings, loadSettings } = require('./settings.cjs');
const { createMiniViewWindow, toggleMiniView } = require('./window.cjs');
const { showRestartPrompt } = require('./dialogs.cjs');
const { 
  forceMemoryOptimization, 
  performGarbageCollection, 
  getCurrentMemoryUsage, 
  getMemoryManagerStats 
} = require('./memory-manager.cjs');
...
function setupIpcHandlers() {
  ...
  ipcMain.on('save-settings', async (event, newSettings) => {
    try {
      ...
      if (newSettings.windowMode !== prevSettings.windowMode) {
        const { setWindowMode } = require('./window.cjs');
  ...
  ipcMain.on('switch-to-tab-handled', (event, tab) => {
    ...
    const { updateTrayMenu } = require('./tray.cjs');
    updateTrayMenu();
  });
  ...
  ipcMain.on('toggle-mini-view', () => {
    ...
    try {
      const { toggleMiniView } = require('./window.cjs');
      toggleMiniView();
    } catch (error) {
  ...
  ipcMain.handle('get-memory-usage', () => {
    try {
      const { getMemoryInfo } = require('./memory-manager.cjs');
  ...
  ipcMain.on('request-gc', (event) => {
    ...
    try {
      ...
      const { performGC } = require('./memory-manager.cjs');
  ...
  ipcMain.on('optimize-memory', (event) => {
    ...
    try {
      ...
      if (nativeModule && typeof nativeModule.optimize_memory === 'function') {
        ...
        try {
          const result = JSON.parse(resultJson);
          const memoryInfo = require('./memory-manager.cjs').getMemoryInfo();
  ...
  ipcMain.handle('check-memory', async () => {
    try {
      const { checkMemoryUsage } = require('./memory-manager.cjs');
