 ...
 * 타이핑 통계 계산 모듈
 */
const { calculateAccuracy, calculateWPM, estimateWordCount, getMemoryInfo } = require('./utils.cjs');
const { reusableBuffers } = require('./memory-manager.cjs');
...
function calculateStatsNormal(data, callback) {
  try {
    ...
    // 주기적으로 메모리 정리
    if (++gcCounter % 10 === 0) {
      const { manualGarbageCollect } = require('./memory-manager.cjs');
