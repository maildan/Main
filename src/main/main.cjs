...
const { setupAppEventListeners } = require('./app-lifecycle.cjs');
...
function setupMemoryManagement() {
  ...
  try {
    // 네이티브 메모리 모듈 초기화
    const { isNativeModuleAvailable, initializeMemorySettings } = require('../server/native.cjs');
...
function setupMemoryOptimizationEvents() {
  ...
  const { isNativeModuleAvailable, optimizeMemory, forceGarbageCollection } = require('../server/native.cjs');
