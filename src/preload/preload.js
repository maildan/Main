...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  toggleMiniView: () => {
    ...
    // 자모 단위 입력 전송 함수
    sendJamo: (jamo) => {
      console.log(`자모 입력 전송: ${jamo}`);
      return ipcRenderer.send('ime-composition-event', {
        type: 'jamo',
        text: jamo,
        timestamp: Date.now()
      });
    ...
    // 자모를 한글로 조합 처리
    processJamo: (jamo) => {
      return ipcRenderer.invoke('process-jamo', jamo);
    },
    ...
    onJamoProcessed: (callback) => {
      ...
      const handler = (_event, data) => {
        console.log('자모 처리 결과:', data);
        callback(data);
      };
      
      ipcRenderer.on('jamo-processed', handler);
      
      return () => {
        ipcRenderer.removeListener('jamo-processed', handler);
      };
    },

    // 한글 IME Composition 완료 이벤트
    onImeCompositionCompleted: (callback) => {
      if (!callback || typeof callback !== 'function') {
        console.error('유효한 콜백 함수가 필요합니다');
        return () => {};
      }
      ...
      const handler = (_event, data) => {
        console.log('IME Composition 완료 이벤트:', data);
        callback(data);
      };
      
      ipcRenderer.on('ime-composition-completed', handler);
      
      return () => {
        ipcRenderer.removeListener('ime-composition-completed', handler);
      };
    },
