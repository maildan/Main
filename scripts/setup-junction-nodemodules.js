/**
 * node_modules 정션 포인트 설정 스크립트
 * 
 * 이 스크립트는 Google Drive와 같은 클라우드 동기화 폴더에서 발생하는
 * ENOTDIR 및 기타 파일 관련 오류를 해결하기 위해 node_modules 폴더를
 * 클라우드 동기화 대상이 아닌 로컬 디렉토리에 생성하고 정션 포인트를 설정합니다.
 * 
 * 정션 포인트는 심볼릭 링크보다 호환성이 더 높으며 Google Drive에서도 작동합니다.
 */
...
async function main() {
  console.log('🔧 node_modules 정션 포인트 설정 도구');
  console.log('이 도구는 node_modules 폴더를 Google Drive 외부에 저장하고 정션 포인트를 생성합니다.\n');

  try {
    // 1. 관리자 권한 확인 (Windows에서 정션 포인트 생성 시 필요)
    const isAdmin = checkAdminRights();
    if (!isAdmin && process.platform === 'win32') {
      console.log('⚠️ Windows에서 정션 포인트를 생성하려면 관리자 권한이 필요할 수 있습니다.');
      console.log('💡 이 스크립트를 관리자 권한으로 실행하세요 (관리자 권한으로 PowerShell/명령 프롬프트 실행 후 node scripts/setup-junction-nodemodules.js)');
    ...
    // 5. 정션 포인트 생성 (Windows 전용)
    console.log('\n🔗 정션 포인트 생성 중...');
    
    try {
      if (process.platform === 'win32') {
        // Windows에서는 mklink /J 명령어 사용 (정션 포인트)
        execSync(`mklink /J "${nodeModulesPath}" "${externalNodeModulesPath}"`, { stdio: 'inherit' });
        console.log('✅ 정션 포인트 생성 완료');
      } else {
        // Unix 계열에서는 ln -s 명령어 사용 (심볼릭 링크)
        execSync(`ln -s "${externalNodeModulesPath}" "${nodeModulesPath}"`, { stdio: 'inherit' });
        console.log('✅ 심볼릭 링크 생성 완료');
      }
    } catch (error) {
      throw new Error(`정션 포인트 생성 실패: ${error.message}`);
    }
    ...
    if (shouldInstall.toLowerCase() !== 'n') {
      return new Promise((resolve) => {
        ...
        npmProcess.on('close', (code) => {
          ...
          console.log(`- 정션 포인트 경로: ${nodeModulesPath}`);
